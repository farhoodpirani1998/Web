...
/**
 * Events "no results" state.
 *
 * Standalone presentation only — this is the visual `EventList`
 * would show in place of the grid if a future filtered/searched
 * event list came back empty (a future `useEvents()`-style data hook
 * with query params, e.g. filtering by category or date range). There
 * is no filter/search state to make empty today, so this component is
 * exported for future wiring but intentionally not composed into
 * `EventsPage` — mirroring the same precedent as
 * `@/features/campuses`'s and `@/features/teachers`'s `EmptyState`.
 *
...
export function EmptyState() {
  return (
    <Section spacing="lg" aria-label="نتیجه‌ای برای رویدادها یافت نشد">
      <SharedEmptyState
        title="رویدادی یافت نشد"
        description="متن نمونه برای زمانی که هیچ رویدادی با معیارهای انتخابی مطابقت ندارد."
