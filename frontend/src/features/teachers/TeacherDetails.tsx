 ...
 *
 * Presentation only, no business logic. Like `CampusDetails` and
 * `FAQ` (both `campuses` and `pre-registration`), each panel uses the
 * native `<details>`/`<summary>` elements rather than a controlled
 ...
 * carries the id `#teacher-{id}`, left in place for a future
 * `TeacherCard` deep-link (mirroring `CampusCard`'s "جزئیات بیشتر"
 * link) — not wired today, since that would mean modifying
 * `TeacherCard`, which is out of this task's scope.
 *
 * Reuses the same local `teachers` literal (`./data`) as `TeacherGrid`
