...
import { deleteCampus } from "./api";
import type { CmsCampus } from "./types";

/**
 * Confirms and performs `DELETE /admin/campuses/:id`. Unlike
 * `features/cms/media/MediaDeleteConfirm.tsx`, there's no 409/"still in
 * use" case to special-case here — `CampusesService.remove` is a plain
 ...
 * as `features/cms/events/EventDeleteConfirm.tsx`/
 * `features/cms/teachers/TeacherDeleteConfirm.tsx`.
 *
 * `campus.title` is `Translatable<string>` (see `types.ts`'s top
 * comment) — rendered with `dir="rtl"`, same as
 * `EventDeleteConfirm`'s `event.title.fa`, unlike
 * `TeacherDeleteConfirm`'s plain `teacher.fullName`.
 */
export interface CampusDeleteConfirmProps {
  campus: CmsCampus;
  onCancel: () => void;
  onDeleted: (id: string) => void;
}

export function CampusDeleteConfirm({ campus, onCancel, onDeleted }: CampusDeleteConfirmProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-6">
        <h2 className="text-lg font-semibold text-slate-900">Delete campus</h2>
        <p className="mt-1 text-sm text-slate-600" dir="rtl">
          This permanently deletes “{campus.title.fa}”. This cannot be undone.
        </p>
