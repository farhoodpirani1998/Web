...
/**
 * Sets/clears `Campus.publishAt` via the dedicated `PATCH
 * /admin/campuses/:id/schedule` action (`scheduleCampus` in
 * `./api.ts`) — distinct from `status`, per the entity's own doc
 * comment: a `published` campus whose `publishAt` is still in the
 * future stays out of the sitemap/public API until that moment. Gated
 * behind `website.content:publish`, same permission
 * `CampusStatusControl` uses, matching `CampusesController.schedule`'s
 * own guard.
 *
 ...
 * dialog weight `CampusForm`/`CampusDeleteConfirm` use. Identical to
 * `features/cms/teachers/TeacherScheduleControl.tsx`/
 * `features/cms/events/EventScheduleControl.tsx`.
 */
export interface CampusScheduleControlProps {
...
export function CampusScheduleControl({
  publishAt,
  isUpdating,
  onSchedule,
}: CampusScheduleControlProps) {
