 ...
 * `TestimonialsService.updateStatus`) is the real enforcement; showing
 * an invalid button here would just fail server-side, not corrupt
 * data. Identical to `FaqStatusControl` — same `PublishStatus` enum.
 */
const VALID_TRANSITIONS: Record<CmsTestimonialStatus, CmsTestimonialStatus[]> = {
  draft: ["published", "archived"],
  published: ["archived", "draft"],
  archived: ["draft"],
};

const STATUS_LABEL: Record<CmsTestimonialStatus, string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};

const STATUS_BADGE_CLASS: Record<CmsTestimonialStatus, string> = {
  draft: "bg-slate-100 text-slate-600",
  published: "bg-emerald-50 text-emerald-700",
  archived: "bg-amber-50 text-amber-700",
};

export interface TestimonialStatusControlProps {
  status: CmsTestimonialStatus;
  isUpdating: boolean;
  onChangeStatus: (status: CmsTestimonialStatus) => void;
}
