 ...
 * `StatisticsService.updateStatus`) is the real enforcement; showing an
 * invalid button here would just fail server-side, not corrupt data.
 */
const VALID_TRANSITIONS: Record<CmsStatisticStatus, CmsStatisticStatus[]> = {
  draft: ["published", "archived"],
  published: ["archived", "draft"],
  archived: ["draft"],
};

const STATUS_LABEL: Record<CmsStatisticStatus, string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};

const STATUS_BADGE_CLASS: Record<CmsStatisticStatus, string> = {
  draft: "bg-slate-100 text-slate-600",
  published: "bg-emerald-50 text-emerald-700",
  archived: "bg-amber-50 text-amber-700",
};

export interface StatisticStatusControlProps {
  status: CmsStatisticStatus;
  isUpdating: boolean;
  onChangeStatus: (status: CmsStatisticStatus) => void;
}

export function StatisticStatusControl({
  status,
  isUpdating,
  onChangeStatus,
}: StatisticStatusControlProps) {
