 ...
 * `HeroService.updateStatus`) is the real enforcement; showing an
 * invalid button here would just fail server-side, not corrupt data.
 */
const VALID_TRANSITIONS: Record<CmsHeroSlideStatus, CmsHeroSlideStatus[]> = {
  draft: ["published", "archived"],
  published: ["archived", "draft"],
  archived: ["draft"],
};

const STATUS_LABEL: Record<CmsHeroSlideStatus, string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};

const STATUS_BADGE_CLASS: Record<CmsHeroSlideStatus, string> = {
  draft: "bg-slate-100 text-slate-600",
  published: "bg-emerald-50 text-emerald-700",
  archived: "bg-amber-50 text-amber-700",
};

export interface HeroSlideStatusControlProps {
  status: CmsHeroSlideStatus;
  isUpdating: boolean;
  onChangeStatus: (status: CmsHeroSlideStatus) => void;
}

export function HeroSlideStatusControl({
  status,
  isUpdating,
  onChangeStatus,
}: HeroSlideStatusControlProps) {
