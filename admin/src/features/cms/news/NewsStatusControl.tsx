 ...
 * `NewsService.updateStatus`) is the real enforcement; showing an
 * invalid button here would just fail server-side, not corrupt data.
 * Identical to `features/cms/faq/FaqStatusControl.tsx` and
 * `features/cms/hero-slides/HeroSlideStatusControl.tsx` — not shared
 * because nothing has promoted it to `features/cms/components/` yet
 * (see that folder's README).
 */
const VALID_TRANSITIONS: Record<CmsNewsStatus, CmsNewsStatus[]> = {
  draft: ["published", "archived"],
  published: ["archived", "draft"],
  archived: ["draft"],
};

const STATUS_LABEL: Record<CmsNewsStatus, string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};

const STATUS_BADGE_CLASS: Record<CmsNewsStatus, string> = {
  draft: "bg-slate-100 text-slate-600",
  published: "bg-emerald-50 text-emerald-700",
  archived: "bg-amber-50 text-amber-700",
};

export interface NewsStatusControlProps {
  status: CmsNewsStatus;
  isUpdating: boolean;
  onChangeStatus: (status: CmsNewsStatus) => void;
}

export function NewsStatusControl({ status, isUpdating, onChangeStatus }: NewsStatusControlProps) {
