 ...
 * `TeachersService.updateStatus`) is the real enforcement; showing an
 * invalid button here would just fail server-side, not corrupt data.
 * Identical to `features/cms/events/EventStatusControl.tsx` and
 ...
 * README).
 */
const VALID_TRANSITIONS: Record<CmsTeacherStatus, CmsTeacherStatus[]> = {
  draft: ["published", "archived"],
  published: ["archived", "draft"],
  archived: ["draft"],
};

const STATUS_LABEL: Record<CmsTeacherStatus, string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};

const STATUS_BADGE_CLASS: Record<CmsTeacherStatus, string> = {
  draft: "bg-slate-100 text-slate-600",
  published: "bg-emerald-50 text-emerald-700",
  archived: "bg-amber-50 text-amber-700",
};

export interface TeacherStatusControlProps {
  status: CmsTeacherStatus;
  isUpdating: boolean;
  onChangeStatus: (status: CmsTeacherStatus) => void;
}

export function TeacherStatusControl({
  status,
  isUpdating,
  onChangeStatus,
}: TeacherStatusControlProps) {
