...
import { TeacherScheduleControl } from "./TeacherScheduleControl";
import { TeacherStatusControl } from "./TeacherStatusControl";
import type { CmsTeacher, CmsTeacherStatus } from "./types";

/**
 * One teacher row in `TeacherList`'s table. Combines two idioms from
 * sibling modules that neither has alone:
 *   - Drag handle plus up/down reorder buttons, same as
 *     `features/cms/faq/FaqRow.tsx` — only rendered when `canReorder`
 *     is true (see `TeacherList`'s comment for why). Up/down buttons
 *     exist alongside native HTML5 drag and drop because dragging
 *     alone isn't keyboard accessible.
 *   - Avatar thumbnail (resolved via `media/useMediaById`, since the
 *     teacher only carries `avatarMediaId`, never an embedded media
 *     object), status control, and schedule control plus a History
 *     button, same as `features/cms/events/EventRow.tsx`.
 *
 * Both drag/up-down and the write actions are gated behind
 * `content:write`, matching `TeachersController.reorder`'s/`update`'s
...
export function TeacherRow({
  ...
  const { media, isLoading } = useMediaById(teacher.avatarMediaId);
    ...
    >
      ...
      <td className="w-16 px-3 py-3 align-top">
        <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-slate-50">
          {teacher.avatarMediaId && isLoading ? (
      ...
      <td className="px-3 py-3 align-top">
        <p className="line-clamp-2 font-medium text-slate-900">{teacher.fullName}</p>
        <p className="mt-0.5 line-clamp-2 text-xs text-slate-500" dir="rtl">
          {teacher.jobTitle.fa}
        </p>
        <p className="mt-0.5 text-xs text-slate-400">/teachers/{teacher.slug}</p>
      </td>

      <td className="px-3 py-3 align-top text-slate-600">
        {teacher.department ? (
          <span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs" dir="rtl">
            {teacher.department.fa}
          </span>
        ) : (
          <span className="text-xs text-slate-400">—</span>
        )}
      ...
      <td className="px-3 py-3 align-top">
        <TeacherStatusControl
          ...
          onChangeStatus={(status) => onChangeStatus(teacher, status)}
        />
      </td>

      <td className="px-3 py-3 align-top">
        <TeacherScheduleControl
          publishAt={teacher.publishAt}
          isUpdating={isUpdatingSchedule}
          onSchedule={(publishAt) => onSchedule(teacher, publishAt)}
        />
      </td>
      ...
      <td className="px-3 py-3 text-right align-top">
        <div className="flex flex-wrap justify-end gap-2">
          ...
          <PermissionGate permission="website.content:write">
            <button
              type="button"
              onClick={() => onDeleteRequest(teacher)}
