...
/**
 * Sets/clears `StaticPage.publishAt` via the dedicated
 * `PATCH /admin/pages/:id/schedule` action (`schedulePage` in
 * `./api.ts`) — identical idiom and identical UI to News'
 * `NewsScheduleControl.tsx` (`PagesService.schedule` mirrors
 * `NewsService.schedule` exactly). Gated behind
 * `website.content:publish`.
 */
export interface PageScheduleControlProps {
...
export function PageScheduleControl({
  publishAt,
  isUpdating,
  onSchedule,
}: PageScheduleControlProps) {
