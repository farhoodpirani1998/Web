...
import { fetchEventRevisions, restoreEventRevision } from "./api";
import type { CmsCalendarEvent, CmsEventRevision } from "./types";

/**
 * Revision history dialog for one event — `GET
 * /admin/events/:id/revisions` plus a restore action, gated behind
 * `website.revisions:view`/`website.revisions:restore` respectively
 * (matching `EventsController.listRevisions`/`restoreRevision`'s own
 * guards). Events is one of the backend's revision-enabled types (see
 * `types.ts`'s top comment).
 *
 * Same plain fixed-overlay modal shape as `EventForm`/
 * `EventDeleteConfirm` (see `features/cms/news/NewsForm.tsx`'s comment
 * on why there's no shared `Dialog` primitive yet). Restoring closes
 * the dialog and hands the restored event back to the caller
 * (`EventsService.restoreRevision` returns the live, updated event —
 * restoring records a new revision itself, non-destructive per
 * `RevisionsService`'s own doc comment).
 */
export interface EventRevisionsPanelProps {
  event: CmsCalendarEvent;
  onClose: () => void;
  onRestored: (event: CmsCalendarEvent) => void;
}

export function EventRevisionsPanel({ event, onClose, onRestored }: EventRevisionsPanelProps) {
  const [revisions, setRevisions] = useState<CmsEventRevision[]>([]);
  ...
  async function handleRestore(versionNumber: number) {
    ...
    try {
      const restored = await restoreEventRevision(event.id, versionNumber);
      onRestored(restored);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Something went wrong. Please try again.");
