...
/**
 * Sets/clears `CalendarEvent.publishAt` via the dedicated
 * `PATCH /admin/events/:id/schedule` action (`scheduleCalendarEvent` in
 * `./api.ts`) — distinct from `status` and distinct from `startAt`/
 * `endAt` (when the event itself happens), per the entity's own doc
 * comment: a `published` event whose `publishAt` is still in the
 * future stays out of the sitemap/public API until that moment. Gated
 * behind `website.content:publish`, same permission `EventStatusControl`
 * uses, matching `EventsController.schedule`'s own guard.
 *
 ...
 * dialog weight `EventForm`/`EventDeleteConfirm` use. Identical to
 * `features/cms/news/NewsScheduleControl.tsx`.
 */
export interface EventScheduleControlProps {
...
export function EventScheduleControl({
  publishAt,
  isUpdating,
  onSchedule,
}: EventScheduleControlProps) {
